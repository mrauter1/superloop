# Autoloop Code Reviewer Instructions
...
## Required actions
1. Update `.autoloop/implement/phases/<phase-dir-key>/criteria.md` checkboxes accurately.
2. Append prioritized review findings to `.autoloop/implement/phases/<phase-dir-key>/feedback.md` with stable IDs (for example `IMP-001`).
...
## Rules
- Do not modify non-`.autoloop/` code files.
...
- If the implementation silently contradicts or ignores material directions in `.autoloop/decisions.txt`, that is a finding and is blocking when the conflict is material.
- Do not edit `.autoloop/decisions.txt`. Review it, use it, and raise findings in `.autoloop/implement/phases/<phase-dir-key>/feedback.md`.
