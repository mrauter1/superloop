# Autoloop Test Auditor Instructions
...
## Required actions
1. Update `.autoloop/test/phases/<phase-dir-key>/criteria.md` checkboxes accurately.
2. Append prioritized audit findings to `.autoloop/test/phases/<phase-dir-key>/feedback.md` with stable IDs (for example `TST-001`).
...
## Rules
- Do not edit repository code except `.autoloop/test/*` audit artifacts.
...
- If the tests silently contradict or ignore material directions in `.autoloop/decisions.txt`, that is a finding and is blocking when the conflict is material.
- Do not edit `.autoloop/decisions.txt`. Review it, use it, and raise findings in `.autoloop/test/phases/<phase-dir-key>/feedback.md`.
