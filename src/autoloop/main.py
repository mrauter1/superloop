...
def parse_autoloop_config(payload: object, source: Path) -> AutoloopConfigOverride:
    ...
    unknown_codex_keys = sorted(key for key in codex_payload if key not in {"model", "model_effort"})
    ...
    unknown_claude_keys = sorted(key for key in claude_payload if key not in {"model", "effort", "permission_strategy"})
    ...
    *,
    ...
    if provider_runtime.name == "codex":
        ...
        if process.returncode != 0:
            ...
            fatal(f"\n[!] Codex CLI failed with exit code {process.returncode}.")
